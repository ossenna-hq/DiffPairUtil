...
- User can see a trace-width-versus-gap graph for differential pair planning with axis units, plot-bounded hover readouts, impedance-based tolerance banding that follows the selected gap range, and a live Track/Gap datapoint that is green inside tolerance or red outside tolerance.
